    ...
    async def get_market_snapshot(self, symbol: str, market: str) -> GatewayResult:
        ...
        if self.auth_fail_snapshot:
            return GatewayResult(
                error="stock-data-gateway authorization failed", auth_failed=True
            )
...
def setup_function() -> None:
    ...
    StubGatewayClient.auth_fail_korean = False
...
def test_korean_gateway_unauthorized_returns_safe_auth_flag(monkeypatch: Any) -> None:
    StubGatewayClient.auth_fail_korean = True
    monkeypatch.setattr(score, "GatewayClient", StubGatewayClient)
