...
def test_fmp_client_returns_clear_authentication_note_on_403() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        assert request.headers["apikey"] == "test-key"
        return httpx.Response(403, json={"error": "restricted"})
    ...
    assert "test-key" not in str(result.error)


def test_fmp_client_strips_key_and_includes_it_on_every_request() -> None:
    ...
    asyncio.run(client.get_balance_sheet("AAPL"))
    asyncio.run(client.get_cash_flow_statement("AAPL"))
    asyncio.run(client.get_key_metrics("AAPL"))
    asyncio.run(client.get_ratios("AAPL"))
    asyncio.run(client.get_quote("AAPL"))
    asyncio.run(client.get_company_profile("AAPL"))

    assert len(requested_paths) == 7


def test_fmp_client_logging_never_exposes_api_key(caplog) -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(403, json={"error": "restricted"})
    ...
    assert "super-secret-fmp-key" not in caplog.text
    assert "api_key_configured=True" in caplog.text


def test_settings_reads_fmp_api_key_from_environment(monkeypatch) -> None:
    from app.core.config import get_settings

    monkeypatch.setenv("FMP_API_KEY", "environment-fmp-key")
    get_settings.cache_clear()

    assert get_settings().fmp_api_key == "environment-fmp-key"
