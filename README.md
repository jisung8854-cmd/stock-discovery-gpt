...
미국 상장 종목(`NASDAQ`, `NYSE`, `AMEX`)은 `FMP_API_KEY`가 있을 때 Financial Modeling Prep의 `/stable` API를 사용합니다. 현재 연동되는 FMP 엔드포인트는 다음과 같습니다.

- `stable/profile?symbol={ticker}`
- `stable/income-statement?symbol={ticker}`
- `stable/balance-sheet-statement?symbol={ticker}`
- `stable/cash-flow-statement?symbol={ticker}`
...
배포 환경에서 FMP 연결 상태는 공개 진단 엔드포인트로 확인할 수 있습니다.

```text
GET /health/fmp
```

이 엔드포인트는 `FMP_API_KEY`가 설정되어 있는지와 가벼운 quote 요청의 성공 여부만 반환합니다. FMP 요청 인증은 공식 지원 `apikey` 헤더를 사용하여 URL에 키가 포함되지 않도록 하며, API 키 값 자체는 응답이나 애플리케이션 로그에 출력하지 않습니다. `401` 또는 `403` 응답을 받으면 다음 진단 문구가 반환됩니다.

```text
FMP authentication failed or endpoint not allowed by plan.
```

이 경우 Render의 `FMP_API_KEY` 환경 변수 값과 현재 FMP 요금제에서 요청 엔드포인트가 허용되는지 확인한 후 서비스를 재배포합니다.
