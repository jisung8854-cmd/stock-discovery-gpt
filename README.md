...
| `STOCK_DATA_GATEWAY_URL` | stock-data-gateway URL. 기본값은 `https://stock-data-gateway.onrender.com` |
| `STOCK_DATA_GATEWAY_BEARER_TOKEN` | 이 서비스가 stock-data-gateway를 호출할 때 사용하는 비밀 Bearer 토큰 |
...
5. **Environment** 탭에 `STOCK_DATA_GATEWAY_URL=https://stock-data-gateway.onrender.com`과 stock-data-gateway가 기대하는 `STOCK_DATA_GATEWAY_BEARER_TOKEN`을 Secret으로 설정합니다.
...
| `STOCK_DATA_GATEWAY_BEARER_TOKEN` | 필수 | stock-data-gateway 호출용 비밀 Bearer 토큰. stock-data-gateway가 기대하는 값과 동일하게 Render Secret으로 설정 |
...
`STOCK_DATA_GATEWAY_BEARER_TOKEN`이 비어 있거나 gateway가 `401`/`403`을 반환하면 `/score`는 토큰이나 upstream 응답 본문을 노출하지 않고 `gateway_auth_failed`와 낮은 데이터 신뢰도를 포함한 partial result를 반환합니다. 이 토큰은 외부 Custom GPT 요청을 인증하는 `ACTION_API_BEARER_TOKEN`과 별도입니다.
...
- [ ] stock-data-gateway가 기대하는 `STOCK_DATA_GATEWAY_BEARER_TOKEN`을 Render Secret으로 설정
