...
| `STOCK_DATA_GATEWAY_BEARER_TOKEN` | stock-data-gateway 요청의 `Authorization: Bearer ...` 헤더에 사용할 비밀 토큰. 코드, 로그, 응답에 노출하지 않습니다. |
...
6. stock-data-gateway에 설정된 값과 동일한 `STOCK_DATA_GATEWAY_BEARER_TOKEN`을 Secret 환경 변수로 설정합니다.
7. Health Check Path를 `/health`로 설정합니다.
8. 배포 후 다음 URL이 정상 응답하는지 확인합니다.
...
| `STOCK_DATA_GATEWAY_BEARER_TOKEN` | 필수 | stock-data-gateway 인증용 비밀 Bearer 토큰. Render Secret으로 설정하고 저장소에 커밋하지 않음 |
...
`ACTION_API_BEARER_TOKEN`은 Custom GPT에서 이 backend로 들어오는 요청을 보호합니다. `STOCK_DATA_GATEWAY_BEARER_TOKEN`은 이 backend가 stock-data-gateway로 보내는 요청을 보호하는 별도 토큰입니다. gateway 토큰이 없거나 gateway가 401/403을 반환하면 `/score`는 crash하지 않고 `gateway_auth_failed`, 낮은 신뢰도, 안전한 note를 반환합니다. 토큰 값이나 upstream 오류 본문은 응답에 포함하지 않습니다. `/health`는 배포 상태 확인을 위해 공개 엔드포인트로 유지됩니다.
...
- [ ] `STOCK_DATA_GATEWAY_BEARER_TOKEN`을 Render Secret으로 설정
